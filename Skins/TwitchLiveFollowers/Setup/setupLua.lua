...
function getLivestreamer()
	local MeasureOBJ = SKIN:GetMeasure('MeasureDetectLivestreamer')
	local PATH = (MeasureOBJ:GetStringValue() )

	if ( string.find(PATH, "Livestreamer") ) then
		SKIN:Bang('!SetOption', 'MeterStep3Text', 'Text', 'Livestreamer is already detected on your system. An icon will appear beside each stream name that you can click to watch in your external player. (Default quality can be changed in the main skin\'s settings)')
	else
		SKIN:Bang('!SetOption', 'MeterStep3Text', 'Text', 'This skin supports Livestreamer. Livestreamer allows you to watch streams in an external video player such as VLC Player. You can download it below, or you can always install it at a later time. (Rainmeter may need to be restarted after installing it)')
		SKIN:Bang('!ShowMeter', 'MeterStep3Link')
	end
		SKIN:Bang('[!ShowMeterGroup Step3]')
		SKIN:Bang('[!UpdateMeter *][!Redraw]')
end

function validateKey(inputToken)
	...
	if not tJSON then
		SKIN:Bang('!SetOption', 'MeterInputLabel', 'Text', 'ERROR: Twitch API Down. Try Again Later.')
		SKIN:Bang('!SetOption', 'MeterInputLabel', 'FontColor', '128,0,0')
	else

		if not tJSON.token.valid then
			SKIN:Bang('!SetOption', 'MeterInputLabel', 'Text', 'Invalid Token. Please try again')
			SKIN:Bang('!SetOption', 'MeterInputLabel', 'FontColor', '128,0,0')

			SKIN:Bang('!SetOption', 'MeterValidInvalid', 'FontColor', 'E6E225')
			SKIN:Bang('!SetOption', 'MeterValidInvalid', 'Text', 'Invalid Token. Please make sure you typed/pasted it correctly.')
			SKIN:Bang('!SetOption', 'MeterValidInvalid', 'Hidden', '0')
		else
			SKIN:Bang('!SetOption', 'MeterInputLabel', 'Text', 'Success! Valid Access Token')
			SKIN:Bang('!SetOption', 'MeterInputLabel', 'FontColor', '0,128,0')

			SKIN:Bang('!SetOption', 'MeterValidInvalid', 'FontColor', 'FFFFFF')
			SKIN:Bang('!SetOption', 'MeterValidInvalid', 'Text', 'Linked Account: '..tJSON.token.user_name)
			SKIN:Bang('!SetOption', 'MeterValidInvalid', 'Hidden', '0')

			SKIN:Bang('!SetOption', 'MeterClickFinishText', 'Hidden', '0')

			local AccessToken = SKIN:GetVariable('AccessToken')
			SKIN:Bang('[!WriteKeyValue Variables isInstalled 1 "#@#SharedVars.inc"]')
			SKIN:Bang('[!WriteKeyValue Variables OAuth_Token '..AccessToken..' "#@#SharedVars.inc"]')

			SKIN:Bang('!SetOption', 'MeterFinishButton', 'LeftMouseUpAction', '[!WriteKeyValue "TwitchLiveFollowers\\Twitch" AutoSelectScreen 1 "#SETTINGSPATH#Rainmeter.ini"][!ActivateConfig "TwitchLiveFollowers\\Twitch"][!Zpos "1" "TwitchLiveFollowers\\Twitch"][!Zpos "0" "TwitchLiveFollowers\\Twitch"][!Move "(#WORKAREAX# + (#WORKAREAWIDTH# / 2 ) - (200 / 2))" "(#WORKAREAY# + (#WORKAREAHEIGHT# / 2 ) - (52 / 2))" "TwitchLiveFollowers\\Twitch"][!DeactivateConfig "TwitchLiveFollowers\\Setup"]')
			SKIN:Bang('!SetOption', 'MeterFinishButton', 'ImageAlpha', '255')
			SKIN:Bang('!SetOption', 'MeterFinishButtonText', 'FontColor', 'FFFFFF')
		end
	end
